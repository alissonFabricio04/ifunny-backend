...
import { Meme, Tag } from '../../../src/domain/entities/meme'
...
test('it should not be able to create a new meme if the number of upvotes is invalid', () => {
  ...
  ).toThrow('Quantidade de upvotes invalida')
})
...
test('it should be able to create a meme', () => {
  ...
  expect(
    Meme.create(new Id(meme.authorId), new Midia(meme.content), tags),
  ).toBeInstanceOf(Meme)
})

test('it should be able to instance a meme', () => {
  const tags: Array<Tag> = []
  for (let i = 0; i < 19; i++) {
    tags.push({ name: 'shitpost', weight: 10 })
  }
...
test('it should be able to upvote the meme', () => {
  ...
  const m = Meme.create(new Id(meme.authorId), new Midia(meme.content), tags)
  m.upvote()
...
test('it should be able to downvote the meme', () => {
  ...
  expect(m.getDownvotes()).toStrictEqual(1)
})
...
test('it should not be able to add new tag if limit has already been reached', () => {
  ...
  expect(() => m.addTag({ name: 'test1', weight: 0 })).toThrow(
...
test('it should be able get id', () => {
  ...
  expect(m.getId()).toStrictEqual(new Id(meme.id))
})
