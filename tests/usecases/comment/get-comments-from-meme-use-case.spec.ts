...
import { GetCommentsFromMemeUseCase } from '../../../src/usecases/comment/get-comments-from-meme-use-case'
...
describe('Suite test for testing get comment from meme use case', () => {
  let meme: Meme
  let comment: Comment
  let gateway: CommentGateway
  let getCommentsFromMemeUseCase: GetCommentsFromMemeUseCase
  ...
  beforeEach(() => {
    ...
    getCommentsFromMemeUseCase = new GetCommentsFromMemeUseCase(gateway)
  })

  test('it should be able get comment from meme', async () => {
    const input = {
      data: {
    ...
    await expect(await getCommentsFromMemeUseCase.handle(input)).toStrictEqual({
      status: 'SUCCESS',
      data: {
        comments: [comment]
      },
