...
describe('Suite test for testing get memes in folder use case', () => {
  ...
  beforeEach(() => {
    ...
    folder = new Folder(
      new Id('e08d894c-0c31-47f0-8add-14ce9470b51e'), 
      'Repubs', 
    ...
    gateway = {
      ...
      highlights: () => new Promise((resolve) => resolve([meme])),
      collective: () => new Promise((resolve) => resolve([meme])),
      getMemeInFolder: () => new Promise((resolve) => resolve([meme])),
    } satisfies MemeGateway

    getMemesInFolderUseCase = new GetMemesInFolderUseCase(gateway)
  })

  test('it should be able get memes in folder', async () => {
    ...
    await expect(await getMemesInFolderUseCase.handle(input)).toStrictEqual({
      status: 'SUCCESS',
      data: {
        memes: [meme]
      },
