import { describe, beforeEach, jest, expect, test } from 'bun:test'

import { Id } from '../../../src/domain/value-objects/id/model/id-value-object'
import { MemeGateway } from '../../../src/domain/entities/meme/gateways/meme-gateway'
import { Meme } from '.../../../src/domain/entities/meme/model/meme'
import { MemesInHighlightsUseCase } from '../../../src/usecases/meme/memes-in-highlight-use-case'
import { Midia } from '../../../src/domain/value-objects/midia/model/midia-value-object'

describe('Suite test for testing memes in highlight use case', () => {
  let meme: Meme
  let gateway: MemeGateway
  let memesInHighlightsUseCase: MemesInHighlightsUseCase

  beforeEach(() => {
    meme = new Meme(
      new Id('d262ab0d-6d32-4a12-a651-06af51c40a69'),
      new Id('5d86c647-3265-4cf4-babb-e5ce353a446a'),
      new Midia('data:image/jpeg;base64,'),
      [
        { name: 'nazismo', weight: 10 },
        { name: 'é', weight: 2 },
        { name: 'uma', weight: 2 },
        { name: 'merda', weight: 10 },
      ],
    )

    gateway = {
      publish: jest.fn().mockImplementationOnce(() => Promise.resolve()),
      find: () => new Promise((resolve) => resolve(meme)),
      recentMemesNotLikedByUser: () => new Promise((resolve) => resolve([meme])),
      lastLikes: () => new Promise((resolve) => resolve([meme])),
      like: jest.fn().mockImplementationOnce(() => Promise.resolve()),
      alreadyLikedMeme: jest.fn().mockImplementationOnce(() => Promise.resolve(true)),
      highlights: () => new Promise((resolve) => resolve([meme])),
      collective: () => new Promise((resolve) => resolve([meme])),
      getMemeInFolder: () => new Promise((resolve) => resolve([meme])),
    } satisfies MemeGateway

    memesInHighlightsUseCase = new MemesInHighlightsUseCase(gateway)
  })

  test('it should be able get memes in highlight', async () => {
    gateway.find = () => new Promise((resolve) => resolve(null))

    const input = {
      data: {
        page: 1,
      },
    }

    await expect(await memesInHighlightsUseCase.handle(input)).toStrictEqual({
      status: 'SUCCESS',
      data: {
        highlights: [meme]
      },
    })
  })
})
