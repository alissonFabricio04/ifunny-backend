...
import { MemesInCollectiveUseCase } from '../../../src/usecases/meme/memes-in-collective-use-case'
import { Midia } from '../../../src/domain/value-objects/midia/model/midia-value-object'

describe('Suite test for testing memes in collective use case', () => {
  let meme: Meme
  let gateway: MemeGateway
  let memesInCollectiveUseCase: MemesInCollectiveUseCase
  ...
  beforeEach(() => {
    ...
    memesInCollectiveUseCase = new MemesInCollectiveUseCase(gateway)
  })

  test('it should be able get memes in collective', async () => {
    ...
    await expect(await memesInCollectiveUseCase.handle(input)).toStrictEqual({
      status: 'SUCCESS',
      data: {
        memes: [meme]
      },
